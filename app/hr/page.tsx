  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6 lg:p-8">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <HrNotificationsBell />
          <Link href="/hr/jobs" className="inline-flex items-center gap-1 rounded-lg border border-border bg-card/60 px-3 py-2 text-sm font-medium transition hover:border-primary/40">
            Jobs <ArrowRight className="h-3.5 w-3.5" />
          </Link>
          <Link href="/hr/candidates" className="inline-flex items-center gap-1 rounded-lg border border-border bg-card/60 px-3 py-2 text-sm font-medium transition hover:border-primary/40">
      ...
      <Card className="border-border bg-card/60 p-6 backdrop-blur">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link href="/hr/jobs" className="text-sm text-primary hover:underline">View all</Link>
        </div>
        {recentJobs.length === 0 ? (
          <div className="py-8 text-center text-sm text-muted-foreground">
            No jobs created yet. <Link href="/hr/jobs" className="text-primary hover:underline">Create your first job</Link>.
          </div>
        ) : (
          <div className="divide-y divide-border">
            {recentJobs.map((j: any) => (
              <Link key={j.id} href={`/hr/jobs/${j.id}`} className="flex items-center gap-3 py-3 transition hover:bg-secondary/50">
