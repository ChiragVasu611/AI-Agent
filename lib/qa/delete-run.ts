...
export interface DeleteRunOutcome {
  ...
  /** Stored evidence objects (screenshots/recordings) removed from the store. */
...
export async function deleteRunCascade(run: {
  ...
  // Screenshot/recording BYTES live in the evidence store, not in the documents
  // just removed, so deleting the rows alone would orphan them on disk or in the
  // bucket forever. Every artefact for a run shares one key prefix, so this is a
  // single sweep. Best-effort: a storage failure must not abort the delete.
  let evidenceObjectsDeleted = 0;
  try {
    const store = await getEvidenceStore();
    evidenceObjectsDeleted = await store.deletePrefix(runPrefix(String(runId)));
  } catch (e) {
    console.error('QA delete-run: could not remove stored evidence for run', String(runId), e);
  }
